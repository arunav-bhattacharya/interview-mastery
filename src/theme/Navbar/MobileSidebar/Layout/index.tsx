...
// in some browsers, leaves the drawer looking empty on the homepage.
//
...
// Pre-React-19 inert attribute compat (matches upstream behaviour)
...
export default function NavbarMobileSidebarLayout({
  ...
  // the value — primary is always visible in this layout.
  ...
  return (
    <div
      ...
      {header}
      <div className={clsx('navbar-sidebar__items', styles.stack)}>
        <MobilePanel className={styles.primary}>{primaryMenu}</MobilePanel>
        <MobilePanel className={styles.secondary}>{secondaryMenu}</MobilePanel>
      </div>
    </div>
  );
}
